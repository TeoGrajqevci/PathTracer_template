...
SceneHit rayMarch(vec3 ro, vec3 rd){
    ...
    SceneHit nh;nh.hit=false;return nh;
}
...
void buildTangentFrame(vec3 N,out vec3 T,out vec3 B){
    if(abs(N.x)>abs(N.z)) {T=vec3(-N.y,N.x,0.0);}else{T=vec3(0.0,-N.z,N.y);}
    T=normalize(T);
    B=cross(N,T);
}
...
vec3 evalBSDF(Material mat,vec3 N,vec3 V,vec3 L,out float pdf){
    ...
    float wD=(diffW>0.0)?0.5:0.0;float wS=0.5;
...
float areaLightPDF(vec3 Lpos,vec3 hp,vec3 N){
    ...
    vec3 ld=Lpos-hp;float dist2=dot(ld,ld);float dist=sqrt(dist2);
...
vec3 traceRay(inout uvec4 seed,vec3 ro,vec3 rd){
    vec3 L=vec3(0.0);vec3 tp=vec3(1.0);
    for(int b=0;b<MAX_BOUNCES;b++){
        SceneHit hit=rayMarch(ro,rd);
        if(!hit.hit) break;
        Material mat=getMaterial(hit.id, hit.blend);
        if(length(mat.emission)>0.0){L+=tp*mat.emission;break;}
        vec3 N=hit.normal;vec3 V=-rd;
        ...
        float npdf;vec3 be=evalBSDF(mat,N,V,ndir,npdf);
        float NdotNext=max(dot(N,ndir),0.0);
        if(npdf<1e-7||NdotNext<EPSILON)break;
        float p=0.9;if(random(seed)>p)break;
        be/=p;
        tp*=be*NdotNext/npdf;
        ro=hit.pos+N*EPSILON;rd=ndir;
    }
...
void main(){
    ...
    float r1=random(seed)-0.5;float r2=random(seed)-0.5;
